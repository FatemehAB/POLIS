...
def heuristic(s):
    ...
    if s[1] < 8: # agent in the first or second lane
      ...
      if s[5] < 8: # lane is empty; increase speed
        action = 3
      else: # avoid collision and still increase speed; also just focus on the nearest car
        if s[4] < s[0]: # car behind me; increase speed
          action = 3    
        else: # car in front of me
          if s[6] < s[2]: # car is slower; decrease speed
            action = 4
          elif (s[4] - s[0]) > 50: # faster car is atleast 50 steps ahead; increase speed
            action = 3
          else: # faster car is less than 50 steps ahead; maintain speed
            action = 1
