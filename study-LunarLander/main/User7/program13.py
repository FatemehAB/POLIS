...
def heuristic(s):
    ...
    if s[4] > 0.05 or s[5] > 0.05 or (s[4] > 0.01 and 0 < s[5] and s[5] < 0.02):
        action = 3
    elif s[4] < -0.05 or s[5] < -0.05 or (s[4] < -0.01 and -0.02 < s[5] and s[5] < 0):
        action = 1
    elif s[1] > 0.5 and s[3] < -0.2:
        action = 2
    elif s[1] <= 0.5 and s[3] < -0.1:
        action = 2
    else:
        action = 0
