...
def heuristic(s):
    ...
    if s[5] < neg(0.5):
        ...
        action = 1
    elif s[4] > 0.2 and (s[1] > 0.2 or s[2] > -0.5):
        action = 3
    elif s[4] < -0.2 and (s[1] > 0.2 or s[2] > -0.5):
        action = 1
    # if to the left of landpad, fire right engine
    elif s[0] > 0.09 and s[4] < 0.03 and s[1] < 0.3:
        action = 1
    # if to right, fire left
    elif s[0] < neg(0.09) and s[4] > neg(0.03) and s[1] < 0.3:
        action = 3
    elif s[2] < -0.01 and neg(1) < s[4] and s[4] < 1:
        action = 2
