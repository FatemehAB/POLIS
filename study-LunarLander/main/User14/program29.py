...
def heuristic(s):
    ...
    if s[1] < 0.3 or s[3] < -1.5:
        action = 2
    # If the ship is tilted, use fire right or left engine to adjust it
    elif s[0] > 0.3 and s[0] < 0.6 and s[5] > 1:
        action = 1
    elif s[0] > -0.6 and s[0] < -0.3 and s[5] < -1:
        action = 3
    else:
        action = 0
