...
def heuristic(s):
    ...
    calc_based_off_angle = 0
    if new_angular_speed > new_horizontal_speed:
      calc_based_off_angle = 1
    else:
      calc_based_off_angle = 0
    if fireengine == 1 and calc_based_off_angle == 1:
      if s[5] < 0: # no angular velocity doesn't mean that you will fall in a perfectly straight line...
        action = 1
      elif s[5] > 0:
        action = 3
      else:
        action = 2
    elif fireengine == 1 and calc_based_off_angle == 0:
      if s[2] < 0:
        action = 1
      elif s[2] > 0:
        action = 3
